...
public class MCTS
{
    ...
    //
    private int HeuristicScore(GameManager.PlayerType[,] board, int row, int col, GameManager.PlayerType currentPlayer,
        int movesMade)
    {
        int score = 0;
            ...
            for (int c = 4; c < BOARD_SIZE; c++)
            {
                ...
                if (count == 4 && emptyR != -1)
                {
                    if ((r == 0 || c == BOARD_SIZE - 1 || board[r - 1, c + 1] != GameManager.PlayerType.None) ||
                        (r + 4 == BOARD_SIZE - 1 || c - 4 == 0 || board[r + 5, c - 5] != GameManager.PlayerType.None))
                        return (emptyR, emptyC);
                }
            }
        }
